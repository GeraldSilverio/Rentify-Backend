...
public sealed class ReservationVehicleResolver : IReservationVehicleResolver
{
    ...
    public ReservationVehicleResolver(IVehicleRepository vehicleRepository)
    {
        _vehicleRepository = vehicleRepository;
    }

    public async Task<Vehicle> GetReservableVehicleAsync(
        Guid tenantId,
        Guid vehicleId,
        CancellationToken cancellationToken)
    {
        ...
        if (!vehicle.IsActive
            || vehicle.Status is VehicleStatus.Maintenance or VehicleStatus.OutOfService)
        {
            throw new ApiException(
    ...
    public VehicleRate GetRateOrThrow(
        Vehicle vehicle,
        RentalType rentalType)
    {
        VehicleRate rate =
            vehicle.Rates.FirstOrDefault(rate =>
                rate.IsActive
                && !rate.IsDeleted
