...
namespace Rentify.Backend.Infrastructure.Persistence.EntityConfiguration.Vehicles;
...
public sealed class VehicleConfiguration : IEntityTypeConfiguration<Backend.Core.Domain.Entities.Vehicles.Vehicle>
{
    public void Configure(EntityTypeBuilder<Backend.Core.Domain.Entities.Vehicles.Vehicle> builder)
    {
        ...
        builder.HasMany(x => x.FeatureAssignments)
            .WithOne(x => x.Vehicle)
            .HasForeignKey(x => x.VehicleId)
            .OnDelete(DeleteBehavior.Cascade);

        builder.Navigation(x => x.FeatureAssignments)
            .HasField("_featureAssignments")
            .UsePropertyAccessMode(PropertyAccessMode.Field);
